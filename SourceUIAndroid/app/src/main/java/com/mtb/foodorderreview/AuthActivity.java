...
//        }
    ...
    private void signUp() {
        ...
        UserService.apiService.register(user).enqueue(new Callback<User>() {
            @Override
            public void onResponse(Call<User> call, Response<User> response) {
                User user1 = response.body();
                Toast.makeText(AuthActivity.this, "đăng ký thành công", Toast.LENGTH_LONG).show();
                if (user1 != null)
                    UserGlobal.getInstance().setData(user1.getId(), user1.getTaiKhoan(), user1.getTen(), user1.getSoDienThoai(), R.drawable.img_app_icon,
                            user1.getEmail(), user1.getDiaChi(), null, false);
                onSuccessHandle();
            ...
            @Override
            public void onFailure(Call<User> call, Throwable t) {
                Toast.makeText(AuthActivity.this, "đăng ký thất bại", Toast.LENGTH_LONG).show();
            }
        });
        // Call your API here
        Utils.UI.disableBtn(AuthActivity.this, auth_sign_up_btn1);
